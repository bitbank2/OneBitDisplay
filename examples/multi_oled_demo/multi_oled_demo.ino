...
rc = obdI2CInit(&obd[0], MY_OLED1, OLED_ADDR, FLIP180, INVERT, 1, SDA_PIN, SCL_PIN, RESET_PIN, 400000L); // use standard I2C bus at 400Khz
  ...
  {
    obdFill(&obd[0], 0, 1);
    obdWriteString(&obd[0], 0,0,0,msgs[rc], FONT_NORMAL, 0, 1);
...
rc = obdInit(&obd[1], MY_OLED2, OLED_ADDR, FLIP180, INVERT, 0, GROVE_SDA_PIN, GROVE_SCL_PIN, RESET_PIN, 400000L); // use standard I2C bus at 400Khz
  if (rc != OLED_NOT_FOUND)
  {
    ...
    obdWriteString(&obd[1], 0,0,0,msgs[rc], FONT_SMALL, 0, 1);
    obdWriteString(&obd[1], 0,4,2,(char *)"Display", FONT_NORMAL, 0, 1);
    obdWriteString(&obd[1], 0,28,3,(char *)"1", FONT_NORMAL, 0, 1);
  }
} /* setup() */
