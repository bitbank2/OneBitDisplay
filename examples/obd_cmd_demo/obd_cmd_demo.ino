...
//  int obdI2CInit(OBDISP *pOBD, int iType, int iAddr, int bFlip, int bInvert, int bWire, int iSDAPin, int iSCLPin, int iResetPin, int32_t iSpeed);
  ...
  obdWriteString(&obd, 0,0,0, (char *)"OBD Command test", FONT_8x8, 0, 1);
  obdWriteString(&obd, 0,0,1, (char *)"Executing...", FONT_8x8, 0, 1);
  obdFill(&obdCMD, 0, 1);
  obdWriteString(&obdCMD, 0,0,0,(char *)"The OBD lib functions", FONT_6x8, 0, 1);
  obdWriteString(&obdCMD, 0,0,1,(char *)"become byte codes", FONT_6x8, 0, 1);
  obdWriteString(&obdCMD, 0,0,2,(char *)"written into memory", FONT_6x8, 0, 1);
  obdWriteString(&obdCMD, 0,0,3,(char *)"and then played back", FONT_6x8, 0, 1);
  obdEllipse(&obdCMD, 32, 48, 31, 15, 1, 1);
  obdEllipse(&obdCMD, 32, 48, 15, 12, 0, 1);
